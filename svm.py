...
def main():
    ...
    # }
    # gs = GridSearchCV(pipeline, param_grid=ps, n_jobs=2, verbose=3)
    ...
    for i, tokens in enumerate(token_ngrams):
        ...
        for offset in offsets:
            ...
            if token_and_label:
                ...
                # features[f'{offset}:token.prefix_3'] = feature_prefix_3(token)
                # features[f'{offset}:token.suffix_3'] = feature_token_suffix_3(token)
                # features[f'{offset}:token.prefix_4'] = feature_prefix_4(token)
                # features[f'{offset}:token.suffix_4'] = feature_token_suffix_4(token)
                features[f'{offset}:token.start_of_sentence'] = feature_start_of_sentence(i)
                # features[f'{offset}:token.end_of_sentence'] = feature_end_of_sentence(i, sentence)
                features[f'{offset}:token.is_short'] = feature_is_short(token)
                # features[f'{offset}:token.is_stopword'] = feature_is_stopword(token)
                ...
                # features[f'{offset}:token.vowel_ratio'] = feature_vowel_ratio(token)
